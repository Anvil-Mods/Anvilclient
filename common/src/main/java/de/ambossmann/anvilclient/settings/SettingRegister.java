...
public class SettingRegister {
	...
	public static final HashMap<Feature, TreeSet<ISetting<?>>> SETTING_LIST_FOR_OPTIONS =
			new HashMap<>();
	...
	private static boolean isSettingField(Field setting) {
		return ISetting.class.isAssignableFrom(setting.getType())
				&& setting.isAnnotationPresent(Setting.class);
	}
	...
	private static void registerFeatureSetting(
			Feature feature, TreeSet<ISetting<?>> settingSet, Field field, Object object) {
		try {
